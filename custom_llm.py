...
# ### My prediction
#
# **Choices.** 3,000 steps at learning rate 0.001, held identical across both
# experiments so the corpus is the only variable that changes. 3,000 steps is roughly
# 12 passes over ~8,000 passages at batch 32 -- enough for a 111k-parameter model to
# fit template regularities, and it runs in about 8 seconds on CPU, so the cost of
# being wrong is trivial. 0.001 is the standard AdamW starting point with warmup and
# cosine decay already in the training cell. A much larger rate (say 0.05) would make
# updates overshoot the minimum and the loss would diverge or oscillate; a much smaller
# one (1e-6) would barely move the weights, and after 3,000 steps the samples would
# still look like the untrained word salad.
#
# **What I expect to change.** Training loss should fall steeply for the first few
# hundred steps and then flatten. Validation loss should track it closely, because the
# synthetic corpus deliberately repeats contexts and held-out passages share templates
# with training ones -- so a small train/validation gap here is evidence of template
# memorization, NOT evidence of generalization. Samples should go from random word
# salad to grammatical-looking template sentences ("the team discussed the ... at the
# ..."). The neighbors of `customer` should shift from arbitrary words toward the other
# role nouns it shares frames with (client, buyer, shopper, consumer, subscriber).
#
# **Eval prediction (starter corpus).** 24 of the 48 cases are out of vocabulary before
# I add anything, so they score 0 automatically and coverage is capped at 24/48. I
# expect the 16 reserved `starter_patterns` cases to improve a lot, since their domain
# associations are taught by hundreds of sibling sentences, and the 8 `starter_transfer`
# cases to improve much less, since they use familiar words in unfamiliar word order.
#
# **Eval prediction (extended corpus).** Coverage should reach 48/48, which I verified
# before training with `verify_corpus.py`. But I expect the headline number to rise
# partly for a trivial reason: 24 cases stop auto-failing and start getting a 1-in-4
# guess, which is ~6 correct from chance alone. So ~26/48 is the null result, not a
# success. I expect `accuracy_scorable_cases` to FALL, because 24 hard cases join a
# denominator that previously held only 24 easy templated ones. I expect grammar and
# categories to be learnable (local n-gram statistics), and `reference` to stay near
# chance, because "maya lent a book to leo . leo thanked ___" requires suppressing the
# most recent name, which is itself one of the four choices.
#
